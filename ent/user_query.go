...
func (uq *UserQuery) QueryRecruitments() *RecruitmentQuery {
	query := &RecruitmentQuery{config: uq.config}
...
func (uq *UserQuery) QueryStocks() *StockQuery {
...
func (uq *UserQuery) QueryApplicants() *ApplicantQuery {
	query := &ApplicantQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRecruitments: uq.withRecruitments.Clone(),
		withStocks:       uq.withStocks.Clone(),
		withApplicants:   uq.withApplicants.Clone(),
...
func (uq *UserQuery) WithRecruitments(opts ...func(*RecruitmentQuery)) *UserQuery {
	query := &RecruitmentQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withRecruitments = query
	return uq
}
...
func (uq *UserQuery) WithStocks(opts ...func(*StockQuery)) *UserQuery {
	query := &StockQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) WithApplicants(opts ...func(*ApplicantQuery)) *UserQuery {
	query := &ApplicantQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withApplicants = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	var (
		...
		_spec       = uq.querySpec()
		loadedTypes = [3]bool{
			uq.withRecruitments != nil,
			uq.withStocks != nil,
			uq.withApplicants != nil,
		}
	)
	...
	if query := uq.withRecruitments; query != nil {
	...
	if query := uq.withStocks; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Stock(func(s *sql.Selector) {
			s.Where(sql.InValues(user.StocksColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := uq.withApplicants; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[string]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
		...
		query.Where(predicate.Applicant(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ApplicantsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
