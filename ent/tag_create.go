...
func (tc *TagCreate) AddRecruitmentTagIDs(ids ...string) *TagCreate {
	tc.mutation.AddRecruitmentTagIDs(ids...)
	return tc
}

// AddRecruitmentTags adds the "recruitment_tags" edges to the RecruitmentTag entity.
func (tc *TagCreate) AddRecruitmentTags(r ...*RecruitmentTag) *TagCreate {
	ids := make([]string, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return tc.AddRecruitmentTagIDs(ids...)
}
...
func (tc *TagCreate) createSpec() (*Tag, *sqlgraph.CreateSpec) {
	...
	if nodes := tc.mutation.RecruitmentTagsIDs(); len(nodes) > 0 {
