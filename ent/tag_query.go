...
func (tq *TagQuery) QueryRecruitmentTags() *RecruitmentTagQuery {
	query := &RecruitmentTagQuery{config: tq.config}
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withRecruitmentTags: tq.withRecruitmentTags.Clone(),
...
func (tq *TagQuery) WithRecruitmentTags(opts ...func(*RecruitmentTagQuery)) *TagQuery {
	query := &RecruitmentTagQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withRecruitmentTags = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withRecruitmentTags; query != nil {
		...
		query.Where(predicate.RecruitmentTag(func(s *sql.Selector) {
			s.Where(sql.InValues(tag.RecruitmentTagsColumn, fks...))
		}))
